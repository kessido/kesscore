...
__all__ = ['zip_cycle_longest', 'lmap', 'lzip', 'lfilter', 'lrange', 'Lmap', 'Lzip', 'Lfilter', 'Lrange', 'isinstance',
           'assert_isinstance', 'compose_star']
...
# Cell
@patch
def assert_all(self:L, checker=noop)->bool:
    for i,v in self.enumerate(): assert checker(v), f'Not all element have evaluated as True. (e.g, element self[{i}]={v})'
...
# Cell
def assert_isinstance(obj, class_or_tuple):
    '''assert `obj` is of type `class_or_tuple`, and o.w raise informative assert message.'''
    assert isinstance(obj, class_or_tuple), f'got instance of type {type(obj)}, but expected {class_or_tuple}'

...
__all__ = ['try_extract', 'split2dirfile', 'download_drive', 'untar_drive', 'download_general', 'download_wget',
           'untar_general', 'untar_wget', 'untar_dropbox']
...
# Cell
def split2dirfile(path):
    spath,ppath = str(path),Path(path)
    if spath[-1:] in '\\/' or ppath.is_dir(): return ppath.parent,ppath.filename
    return ppath,None

# Cell
def download_drive(fid, fname):
    root,filename = split2dirfile(fname)
